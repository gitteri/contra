...
interface WithdrawSectionProps {
  ...
  contraBalance: TokenBalance | null;
  mintAddress: string;
...
function WithdrawSection({
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      // Sign and send the transaction to Contra
  ...
  const handleMaxClick = () => {
    if (contraBalance) {
      setWithdrawAmount(contraBalance.amount);
    }
  };
...
function TransferSection({
  ...
  const handleMaxClick = () => {
    if (contraBalance) {
      setTransferAmount(contraBalance.amount);
    }
  };

  return (
    <div className="function-section">
      <h3>Transfer on Contra</h3>
      <p className="info-text">
...
export function ContraManagement() {
  const { network } = useCluster();
  const [mintAddress, setMintAddress] = useState("");
  const [contraBalance, setContraBalance] = useState<TokenBalance | null>(null);
  ...
  const fetchContraBalance = async () => {
    ...
    try {
      ...
      if (tokenAccountBalance.value) {
        ...
        setError("Token account not found on Contra or balance is zero");
      }
    } catch (err) {
      console.error("Error fetching Contra balance:", err);
      setError(
        err instanceof Error
          ? err.message
          : "Failed to fetch balance from Contra"
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="card">
      ...
      <div className="function-section">
        <h3>Check Balance on Contra</h3>
        <p className="info-text">
          Enter a token mint address to check your balance on Contra
        </p>
        <div className="form-group">
      ...
      {account && walletAddress && contraBalance && (
        <>
          <TransferSection
            account={account}
            chainId={chainId}
